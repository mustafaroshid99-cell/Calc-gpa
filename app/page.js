...
export default function Home() {
  ...
  const start = () => {
    if (!examName || !subjectCount) return alert("Fill setup");
  ...
  const updateMark = (i, v) => {
    if (v === "") return;
    if (Number(v) > 100 || Number(v) < 0) return alert("0-100 only");
    const c = [...marks];
    c[i] = v;
    setMarks(c);
  };
  ...
  const addStudent = () => {
    if (!name || !roll) return alert("Fill name & roll");
    const nums = marks.map(Number);
    if (nums.some((m) => isNaN(m))) return alert("Fill marks");
  ...
  // PDF EXPORT
  const exportPDF = () => {
    const content = merit.map(s => `#${s.rank} ${s.name} (Roll ${s.roll}) GPA: ${s.gpa}`).join("\n");
  ...
  return (
    <div style={dark ? styles.bgDark : styles.bgLight}>
...
const styles = {
  bgDark:{minHeight:"100vh",padding:20,background:"radial-gradient(circle,#020617,#0f172a)",color:"#e2e8f0"},
  bgLight:{minHeight:"100vh",padding:20,background:"#f1f5f9",color:"#020617"},
